...
pileometh_bedgraph = open(methyl_path, 'r')
...
for row in pileometh_summary:
    ...
    if min_coverage and total_reads >= min_coverage:
        coge_formatted_filtered.writerow(
            [chrm, pos, pos, 1, dec_met, total_reads])  # Output as filtered .csv file
    if unfiltered == 't':
        coge_formatted_unfiltered.writerow(
            [chrm, pos, pos, 1, dec_met, total_reads])  # Output as unfiltered .csv file
