...
while (<IN>)
  {
    ...
    next if $line[2] eq "gene"; #duplicated by mRNA
    ...
    $line[2] = "mRNA" if $line[2] eq "5'-UTR";
    $line[2] = "mRNA" if $line[2] eq "five_prime_utr";
    #phytozome:  3'-UTR transformed to mRNA
    $line[2] = "mRNA" if $line[2] eq "3'-UTR";
    $line[2] = "mRNA" if $line[2] eq "three_prime_utr";
	...
	$annos{$name}{"$type: $info"}={} if $anno_names{$type};
	#add phytozome link:
	$annos{$name}{"View annotation at Phytozome"}={link=>$phyto_link."$info", type=>"Phytozome link"} if $type eq "PACid";
      }
    next unless $name; #No name, don't know what to do!
...
#    push @type, "mRNA" if $type eq "CDS";
	...
	foreach my $n (keys %names)
	  {
	    $data{$line[1]}{$name}{$type}{names}{$n}=1;
	    foreach my $n1 ( keys %{$master_names{$n}})
	      {
		$data{$line[1]}{$name}{$type}{names}{$n1}=1
	      }
	  }
      }
#    print Dumper \%data;
