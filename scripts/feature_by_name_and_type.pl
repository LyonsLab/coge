...
while (my $line = <ACCN>){
    ...
    foreach my $feat ( $s->resultset('Feature')->search(
        ...
        if( scalar(grep { $seen{substr(uc($_->name),0,9)} } $feat->feature_names)){ next; }
        my @names = sort grep { $_ =~ /^AT\dG\d{5}\.\d$/i } map { $_->name } $feat->feature_names;
        my $name = @names[0];
        if (!$name){  
            @names = sort grep { $_ =~ /^AT\dG\d{5}$/i } map { $_->name } $feat->feature_names;
        ...
        map { $seen{substr(uc($_->name),0,9)}=1 }  $feat->feature_names; 
        #map { print $_  . "\t" } @names;
