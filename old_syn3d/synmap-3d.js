...
function wait_to_search (search_func, search_obj) {
    ...
    if (search_term && search_term.length >= 3) {
        ...
        pageObj.time = setTimeout(
            function() {
                search_func(search_obj.value, search_obj.id);
            },
            250
        );
    }
...
function search_genomes (search_term, genome_id) {
    $.ajax({
        data: {
        ...
        success : function(data) {
            ...
            if (obj.items) {
                ...
                $("#" + genome_id).autocomplete({source: obj.items});
                $("#" + genome_id).autocomplete("search");
            }
        }
    });
}
...
$.extend(GenomesView.prototype, {
	...
	this.genomeX = this.el.find("#genomeX");
	this.genomeY = this.el.find("#genomeY");
	this.genomeZ = this.el.find("#genomeZ");
        this.get_sources();
        ...
        if (this.metadata) {
            ...
            this.el.find('#genomeX').val(this.metadata.genome);
            this.el.find('#genomeY').val(this.metadata.genome);
            this.el.find('#genomeZ').val(this.metadata.genome);
	 }
    },
    ...
    render: function() {
        var self = this;
	var genomeX = this.genomeX;
	var genomeY = this.genomeY;
	var genomeZ = this.genomeZ;
	//var genomes = [this.genomeX, this.genomeY, this.genomeZ];
        
	// jQuery Events
        genomeX.unbind().change(function() {
            // Reset gid when item has changed
            self.Xgid = undefined;
        });
        genomeY.unbind().change(function() {
            // Reset gid when item has changed
            self.Ygid = undefined;
        });
        genomeZ.unbind().change(function() {
            // Reset gid when item has changed
            self.Zgid = undefined;
        });

        // jQuery UI
        genomeX.autocomplete({
            source:[],
            select: function(event, ui) {
                $(this).val(ui.item.label);
                self.Xgid = ui.item.value;
        ...
        genomeY.autocomplete({
            source:[],
            select: function(event, ui) {
                $(this).val(ui.item.label);
                self.Ygid = ui.item.value;
                return false; // Prevent the widget from inserting the value.
            },
        ...
        genomeZ.autocomplete({
            source:[],
            select: function(event, ui) {
                $(this).val(ui.item.label);
                self.Zgid = ui.item.value;
                return false; // Prevent the widget from inserting the value.
            },
    ...
    is_valid: function() {
        ...
        var genomeX = this.el.find('#genomeX').val();
	var genomeY = this.el.find('#genomeY').val();
	var genomeZ = this.el.find('#genomeZ').val();
        var genomes = [genomeX, genomeY, genomeZ]; 
	/*if (!name) {
        ...
        if (!genomeX || genomeX === 'Search' || !this.Xgid) {
            error_help('Please specify an X-axis genome.');
            return false;
        }
        
	if (!genomeY || genomeY === 'Search' || !this.Ygid) {
            error_help('Please specify a Y-axis genome.');
            return false;
        }
        
	if (!genomeZ || genomeZ === 'Search' || !this.Zgid) {
            error_help('Please specify a Z-axis genome.');
            return false;
        }
...
//AKB ADDED AnalysisOptionsView
function AnalysisOptionsView() {
    this.data = {};
    this.initialize();
}

$.extend(AnalysisOptionsView.prototype, {
    initialize: function() {
        this.el = $($("#analysis-options-template").html());
	this.min_ch_len = this.el.find("#min_ch_len");
        this.hide_no_synteny = this.el.find("#hide_no_synteny");
        //this.edit_notebook = this.el.find("#edit_notebook");
        //this.notebook_container = this.el.find("#notebook-container");
    },
    ...
    is_valid: function() {
        ...
        //    return false;
        //}
	this.data.min_ch_len = this.min_ch_len.val();
	this.data.hide_no_syn = this.hide_no_synteny.is(":checked");
        return true;
    },
...
$.extend(OptionsView.prototype, {
	...
	this.analysis_view = new AnalysisOptionsView();
...
function initialize_wizard(opts) {
    ...
    wizard.addStep(new GenomesView({
