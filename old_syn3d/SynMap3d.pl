...
sub generate_body {
    ...
    # Set X genome ID if specified
    my $Xgid = $FORM->param('Xgid');
    if ($Xgid) {
        my $genome = $coge->resultset('Genome')->find($Xgid);
    ...
    # Set Y genome ID if specified
    my $Ygid = $FORM->param('Ygid');
    if ($Ygid) {
        my $genome = $coge->resultset('Genome')->find($Ygid);
        if ($genome && $USER->has_access_to_genome($genome)) { # check permission
            $template->param(
    ...
    # Set Zgenome ID if specified
    my $Zgid = $FORM->param('Zgid');
    if ($Zgid) {
        my $genome = $coge->resultset('Genome')->find($Zgid);
        if ($genome && $USER->has_access_to_genome($genome)) { # check permission
            $template->param(
