...
use vars qw($P $DBNAME $DBHOST $DBPORT $DBUSER $DBPASS $connstr $PAGE_NAME $TEMPDIR $USER $DATE $BASEFILE $coge $cogeweb $FORM $URL $TEMPURL $MAUVE $COGE_MAUVE $MAUVE_MATRIX $COOKIE_NAME);
...
sub run_alignment
  {
    ...
    CoGe_dev::Accessory::Web::write_log("Running $cmd", $cogeweb->logfile);
    my $output;
    open (CMD, "$cmd |");
    ...
    CoGe_dev::Accessory::Web::write_log("LOG FROM $COGE_MAUVE", $cogeweb->logfile);
    CoGe_dev::Accessory::Web::write_log("########", $cogeweb->logfile);
    CoGe_dev::Accessory::Web::write_log("$output", $cogeweb->logfile);
