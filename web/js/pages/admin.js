...
function init_loadHist(element) {
        $("#load_hist").empty();
	$("#selectedtype_chart").empty();
	console.log(element);
   	var selectedType = document.getElementById("dropType").selectedOptions[0].text;
        $("#loading7").show();
	console.log("init_loadHist",selectedType);
	$.ajax({
		url: "Admin.pl?jquery_ajax=1&fname=get_jobs&status=completed",	
		success: function (json) {
			console.log(json);
			var timeData = JSON.parse(json);
			var finalData = timeData.data;
			console.log(finalData);
			for(var i = 0; i < finalData.length - 1; i++) {
				var statusArray = finalData[i][7];
				var jobType = finalData[i][5];
				if (statusArray !== "Completed") {
					continue;
				}
				if (typeof completedJobs[jobType] == "undefined") {
					completedJobs[jobType] = [];}
					var timeArray = finalData[i][3];
					var splitTime = timeArray.split(' ');
					var totalTime = 0;
					for (var k = 0; k < splitTime.length - 1; k++) {
						var splitArray = splitTime[k];
						if (splitArray)
							var type = splitArray.slice(-1);
						if (type == 'd') {
							totalTime += parseInt(splitArray.slice(0,-1))*1440;
						} else if (type == 'h') {
							totalTime += parseInt(splitArray.slice(0,-1))*60;
						} else if (type == 'm') {
							totalTime += parseInt(splitArray.slice(0,-1));
						} else if (type == 's') {
							totalTime += parseInt(splitArray.slice(0,-1))/60;}
						}
					if (totalTime == 0) {
						completedJobs[jobType].push(1)
					} else {
						completedJobs[jobType].push(totalTime + 1);}
					}		
					console.log(completedJobs[jobType]);
			var values = completedJobs[selectedType];
			if (typeof values !== "undefined") {
				console.log(element);
	                	console.log(values);
        	       		var totalLoads = values.length;
                		console.log(totalLoads);
                		var max_value = Math.max.apply(Math, values);
                		var min_value = Math.min.apply(Math, values);
                		console.log(min_value);
                		console.log(max_value)
				load_hist = new loadHist(element, {min: min_value, max: max_value, values: values}, null);
				$("#selectedtype_chart").show(load_hist);
                        	$("#loading7").hide();
			} else {
		       		$("#loading7").hide();
				$("#selectedtype_chart").append(noJobs());
			}
		}
	});
};
...
function noJobs() {
	var message = "There are no available jobs.";
	return message;
};
 
var loadHist = function(element, json, parent) {
	console.log(element);
	console.log(json);
	console.log(parent);
	var self = this;
	...
	this.max_value = this.json.max;
	console.log(this.max_value);
	this.margin = {top: 30, right: 80, bottom: 50, left: 80};
        this.width = $(window).width()*.75 - this.margin.left - this.margin.right,
        this.height = $(window).height()*.5 - this.margin.top - this.margin.bottom;
...
initialize: function() {
		...
		$("#" + this.element).html(
                        '<div><button id="' + self.element + '_back_button" class="ui-button ui-corner-all coge-button" style="margin-right:' + (this.width/2 - 150) + 'px;">Zoom Out</button>' + 
   				'<div><br></div>'+
			        '<div id="' + self.element + '_container" style="float:right;height:' + (self.height + 500) + 'px;"> </div> </div>'
		);
                $('#' + this.element + '_back_button').on("click", function() {
                        self.zoom_out.call(self);
                });
		...
		this.formatCount = d3.format(",.0f");
		this.formatTime = d3.time.format("%H:%M");
		this.formatMinutes = function(d) { return self.formatTime(new Date(2012, 0, 1, 0, d)); };        
...
// Generate a histogram using twenty uniformly-spaced bins.
		...
		this.brush = d3.svg.brush()
      	 	...
      	 	this.svg = d3.select("#selectedtype_chart").append("svg")
        	...
        	this.bar.append("rect")
                ...
                .attr("height", function(d) { return height - self.y(d.y); });
		...
		.text("Jobs");
        	...
        	var y_label = this.svg.append("text")
                .attr("transform", "translate(" + margin.left/(-1.25) + "," + height/2 + ") rotate(270)")
                .text("Frequency");
	...
	self.init_load_table();

	},
	init_load_table: function() {
		var self = this;
		console.log(this);
		console.log(this.data);
		var totalLoads = 0;
		...
		var totalMemory = 0;
		var totalCount = 0;
		var minMemory = Number.MAX_SAFE_INTEGER;
		var maxMemory = 0;
		...
		var min = self.values.min();
		var max = self.values.max();
		var jobsCount = self.values.length;
		var sumJobs = 0;
			for(var i = 0; i <self.values.length; i++) {
				sumJobs += self.values[i];
			}
		console.log(sumJobs);
		var avgJobs = Math.round(sumJobs/self.values.length);
		...
		console.log(avgJobs);
                ...
                $("#" + self.element + "_container").append("<div style='width:100%'>" +
                                "<div># of Jobs: " + jobsCount + "</div>" +
                                "<div>Minimum Job Time: " + min + " minute(s)</div>" +
                                "<div>Maximum Job Time : " + max + " minute(s)</div>" +
                                "<div>Average Job Time: " + avgJobs + "</div>" 
		);
	},
     	...
     	brushed: function() {
        	var self = this;
             	var extent = this.brush.extent();
              	...
              	var parent = self;
		$("#selectedtype_chart").empty();
              	self = new loadHist(self.element, newJson, parent);
