...
sub dispatch {
	...
	my $fname = $args{'fname'};
	if ($fname) {
		die "AJAX function '$fname' not found" if not defined $FUNCTION{$fname};
...
sub gen_body {
	...
	push @$feat_list, @{ get_fids(lid => $lid, dsid => $dsid, dsgid => $dsgid, ftid => $ftid, chr => $chr, start => $start, stop => $stop) } if ($dsid || $dsgid || $lid);
...
sub send_to_list    #send to list
{
	...
	foreach my $accn (split(/,/, $accn_list)) {
		next if $accn =~ /no$/;
		my ($fid) = split('_', $accn);
		my $conn = $coge->resultset('ListConnector')->create( { parent_id => $list->id, child_id => $fid, child_type => 4 } ); #FIXME hardcoded type!
		return unless $conn;
	}
