...
use vars
  qw($P $DBNAME $DBHOST $DBPORT $DBUSER $DBPASS $connstr $PAGE_NAME $DATE $DEBUG $BL2SEQ $BLASTZ $LAGAN $CHAOS $DIALIGN $GENOMETHREADER $TEMPDIR $TEMPURL $USER $FORM $cogeweb $BENCHMARK $coge $NUM_SEQS $MAX_SEQS $MAX_PROC %FUNCTION $COOKIE_NAME);
...
"<tr><td>Aligned Sequence:<td><span style=\"white-space: nowrap\">"
			...
			$seq =~ s/-//g if $seq;
			$desc .=
			  "<tr><td>Sequence:<td><span style=\"white-space: nowrap\">" . $seq
			  . "</span>"
			  if $seq;
			$desc .=
...
sub run_lagan {
	...
	for ( my $i = 0 ; $i < scalar @$sets ; $i++ ) {
		for ( my $j = 0 ; $j < scalar @$sets ; $j++ ) {
			...
			system "/bin/chmod +rw $tempfile";
			my $report =
			  new CoGe::Accessory::lagan_report(
...
sub run_chaos {
	...
	for ( my $i = 0 ; $i < scalar @$sets ; $i++ ) {
		for ( my $j = 0 ; $j < scalar @$sets ; $j++ ) {
			...
			system "/bin/chmod +rw $tempfile";
			my $report =
			  new CoGe::Accessory::chaos_report(
				{ file => $tempfile, %$parser_opts } )
			  if -r $tempfile;
			my @tmp = ( $tempfile, $accn1, $accn2 );
			if ($report) {
...
"Your search parameters were altered due to the length of the sequences you requested for alignment. Your alignment was done with the \"Large Sequence Alignment\" option enabled";
			...
			if ( $parser_opts->{anchor_params} ) {
				...
				my ( $y, $anchor_opts ) =
				  CoGe::Accessory::Web::check_taint(
				...
				system "/bin/chmod +rw $tempfile";
				$report =
				  new CoGe::Accessory::dialign_report(
					{ file => $tempfile, %$parser_opts } )
				  if -r $tempfile;
			}
			my @tmp = ( $tempfile, $accn1, $accn2 );
...
#	    foreach my $item ({genomic=>$seqfile1, protein=>$protfile2},{genomic=>$seqfile2, protein=>$protfile1})
			foreach my $item ( { genomic => $seqfile2, protein => $protfile1 } )
			{
				...
				system "/bin/chmod +rw $tempfile";
				my $report =
				  new CoGe::Accessory::GenomeThreader_report(
					{ file => $tempfile, %$parser_opts } )
				  if -r $tempfile;
...
sub dataset_search {
	...
	$accn =~ s/^\s+// if $accn;
	$accn =~ s/\s+$// if $accn;
	print STDERR "dataset_search: accn=$accn num=$num dsid=$dsid featid=$featid dsgid=$dsgid gstid=$gstid\n";
	...
	if ($accn) {
		...
		while ( my $ds = $rs->next() ) {
			...
			foreach my $seqtype ( $ds->sequence_type ) {
				...
				$sources{ $ds->id } = { title => $title, version => $ver, typeid => $typeid };
			}
		}
	}
	...
	if ( keys %sources ) {
		...
		# };
		$html .= qq{<SELECT name = "dsid$num" id= "dsid$num" onChange="genome_search(['args__dsid', 'dsid$num', 'args__dsgid', 'dsgid$num', 'args__gstid', 'gstid$num', 'args__num','args__$num', 'args__featid', 'featid$num'],[feat_search_chain]);">};
	...
	return encode_json({html => $html, num => $num, featid => $featid});
}
