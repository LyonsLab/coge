...
sub gen_html
  ...
  sub gen_body
  {
    ...
    $template->param(SEQ=>get_sequence(fid=>$featid,
    								   chr=>$chr,
    								   start=>$upstream,
    								   start=>$upstream,
    								   stop=>$downstream,
...
sub run_aragorn
{
	...
	my ($output_parse,$excel_file) = $format !~ /none/ ? parse_deliminated($output) : ("","");
	
	$excel_file =~ s/$TEMPDIR/$TEMPURL/ if $excel_file;
	
	my $outfile_parse = save_file($output_parse) if $output_parse;
	
	my $html = qq{<div align=center><pre><div align=left class=resultborder style="overflow:auto;width:700px;max-height:700px;"><br/>$output</div></pre>};
	
	$outfile =~s/\/opt\/apache//;
	
	$html .= qq!Download output: <a href="$outfile">Aragorn Output File</a>!;
	$html .= qq!<br>Download tab deliminated output: <a href="$outfile_parse">Tab Deliminated Output</a>! if $outfile_parse;
	$html .= qq!<br>Download Output as Excel File: <a href="$excel_file">Excel Formatted Output</a>! if $excel_file;
	$html .= "</div><br><br>";
	
	if($email !~ /undefined/)
	{
		email_results(email=>$email,raw=>$outfile,tabbed=>$outfile_parse,excel=>$excel_file);
	}
...
sub parse_deliminated
{
	...
	foreach my $ds (@ds)
	{
		...
		{
			my ($num,$type,$loc,$antiloc,$anticodon,$intron) = $lines[$i] =~ /(\d+)\s+(tRNA-\w+)\s+(c?\[\d+,\d+\])\s+(\d+)\s+(\(\w{3}\))(i\(\d+,\d+\))?/;
			$intron = "none" unless $intron;
			my $prim_seq = $lines[$i+1];
			my $second_seq = $lines[$i+2];
			push @$trna_data, {num=>$num,type=>$type,loc=>$loc,antiloc=>$antiloc,anticodon=>$anticodon,intron=>$intron,prim_seq=>$prim_seq,sec_seq=>$second_seq};
		}
	push @results, {name=>$ds_name,gene_no=>$gene_no,results=>$trna_data};
	}
	my $data;
	...
	foreach my $result (@results)
	{
		...
		foreach my $trna (@{$result->{results}})
		{
			$data .= $trna->{num}."\t".$trna->{type}."\t".$trna->{loc}."\t".$trna->{antiloc}."\t".$trna->{anticodon}."\t".$trna->{intron}."\t".$trna->{prim_seq}."\n";
			$data .= "\t\t\t\t\t\t".$trna->{sec_seq}."\n";
		}
	}
