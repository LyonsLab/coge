...
sub gen_html
  {
    ...
    $template->param(PAGE_TITLE=>qq{SeqType});
    $template->param(LOGO_PNG=>"SeqType-logo.png");
...
sub gen_body
  {
      my $template = HTML::Template->new(filename=>$P->{TMPLDIR}.'SeqType.tmpl');
      $template->param(PAGE_NAME=>$FORM->url);
      my @types;
      foreach my $st ($coge->resultset('GenomicSequenceType')->all())
	{
	  my $count = $st->dataset_groups->count();
	  push @types, {
			ID=>$st->id,
			NAME=>$st->name,
			DESC=>$st->description,
