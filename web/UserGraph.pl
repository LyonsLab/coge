...
sub get_all_nodes {
	...
	my %childrenByUser;
	foreach my $conn ($coge->resultset('UserConnector')->search({ 
		parent_type => $node_types->{user}, 
		-or => [ child_type => $node_types->{genome}, child_type => $node_types->{experiment}, child_type => $node_types->{list} ] })) 
	{
		if ($conn->child_type == $node_types->{list}) {
			push @{$childrenByUser{$conn->parent_id}}, 
				{ name => $conn->child_id, type => $conn->child_type, children => $childrenByList{$conn->child_id} };
		}
		else {
			push @{$childrenByUser{$conn->parent_id}}, 
				{ name => $conn->child_id, type => $conn->child_type };
	...
	foreach my $user ($coge->resultset('User')->all) {
		push @users, 
			{ name => $user->id, type => 5, info => $user->info, children => $childrenByUser{$user->id} };
	}
	...
	foreach my $group ($coge->resultset('UserGroup')->all) {
		...
		push @groups, 
			{ name => $group->id, type => 6, info => $group->info, size => $num_users*1000, children => $childrenByGroup{$group->id} };
	}	
...
sub get_node {
	...
	if ($type eq 'user') {
		foreach my $conn ($coge->resultset('UserConnector')->search({parent_id => $id, parent_type => $node_types->{user} })) {
			my $child = $conn->child;
			...
			push @children, { id => $id, name => $name, info => $info, type => $type };
		}
	}
	elsif ($type eq 'group') {
		...
		foreach my $conn ($coge->resultset('ListConnector')->search({parent_id => $id})) {
			my $child = $conn->child;
			my $type = get_type_name($conn->child_type);
			my $name = ($child->name ? $child->name : '<unnamed>');
			my $info = ($child->info ? $child->info : '<no info>');
