...
use vars qw($P $DBNAME $DBHOST $DBPORT $DBUSER $DBPASS $connstr $DATE $DEBUG $TEMPDIR $TEMPURL $USER $FORM $DIAGSDIR $coge);
...
sub gen_body
  {
    my $form = shift || $FORM;
    my $template = HTML::Template->new(filename=>$P->{TMPLDIR}.'SynSub.tmpl');
    ...
    my ($dna_matrix1, $dna_matrix2) = gen_matrix(freq=>$freq_dna, org1_counts => $org1_dna_counts, org2_counts=>$org2_dna_counts);
    my ($prot_matrix1, $prot_matrix2) = gen_matrix(freq=>$freq_prot, org1_counts => $org1_prot_counts, org2_counts=>$org2_prot_counts);
...
sub get_counts
  {
	...
	for (my $i=0; $i < $dna_length; $i+=3) #getting codons
	  {
	    ...
	    next if $chr1 =~ /[^ATCG]/;
	    next if $chr2 =~ /[^ATCG]/;
	    $freq_dna{$chr1}{$chr2}++; #increment frequency count of align characters
	  }
	...
	for (my $i=0; $i < $prot_length; $i+=1) #getting amino acids
	  {
	    ...
	    next if $chr1 =~ /-/;
	    next if $chr2 =~ /-/;
	    $freq_prot{$chr1}{$chr2}++; #increment frequency count of align characters
	  }
      }

    foreach my $c1 (keys %freq_dna)
      {	if (length ($c1) != 3) #sometimes, something goes terribly, terribly wrong
	  {
	    delete $freq_dna{$c1};
	    next;
	  }
	foreach my $c2 (keys %{$freq_dna{$c1}})
	  {
	    if (length ($c2) != 3) #sometimes, something goes terribly, terribly wrong
	      {
    ...
    my $org1_percent_gc_cds = sprintf("%.2f",100*$org1_cds_gc/$org1_cds_total);
    my $org2_percent_gc_cds = sprintf("%.2f",100*$org2_cds_gc/$org2_cds_total);
    my $org1_percent_gc_wobble = sprintf("%.2f",100*$org1_wobble_gc/$org1_wobble_total);
    my $org2_percent_gc_wobble = sprintf("%.2f",100*$org2_wobble_gc/$org2_wobble_total);

    return \%freq_dna, $org1_dna_counts, $org2_dna_counts, $total_dna_counts, \%freq_prot, $org1_prot_counts, $org2_prot_counts, $total_prot_counts, $org1_percent_gc_cds, $org2_percent_gc_cds, $org1_percent_gc_wobble, $org2_percent_gc_wobble;
  }
...
sub gen_matrix
  {
    ...
    my %q; #count of $chr1 changing to $chr2 divided by the total number of aligned characters $total_pairs
    my @chrs = keys %$freq;
    foreach my $c1 (@chrs)
      {
	foreach my $c2 (@chrs)
	...
	foreach my $c2 (@chrs)
	  {
	    ...
	    next unless $q;
	    my $denom = ($p1{$c1}*$p1{$c2}); #combined frequency of $c1 * $c2
	    my $val = sprintf("%.2f",2*log($q/$denom)) if $denom;
	    $val = 0 if $val eq "-0";
	    $matrix1{$c1}{$c2} = $val;
	    $denom = ($p2{$c1}*$p2{$c2}) if defined $p2{$c1} && defined $p2{$c2}; #combined frequency of $c1 * $c2
...
sub gen_matrix_output_html
  {
	...
	@order = sort { sort_nt1(substr($a, 1, 1)) <=> sort_nt1(substr($b,1, 1)) || sort_nt2(substr($a,0,1)) <=> sort_nt2(substr($b,0,1)) || sort_nt3(substr($a,2,1)) <=> sort_nt3(substr($b,2,1)) } @codons;
      }
    else
      {
