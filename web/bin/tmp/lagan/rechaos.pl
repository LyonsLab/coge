...
#}
###THIS IS LENGTH OF ENTIRE SEQUENCE###
...
# PRINT OUT LIST OF REGIONS TO ALIGN
    ...
    open (PFILE, ">$tempdir/$$.anchs.pairs");
    for ($j = 0; $j < @b1; $j++) {
...
# PERFORM THE ALIGNMENTS USING CHAOS
    ...
    $saver = "$lagandir/chaos $seq1 $seq2 $extraparams -wl $wordlen -nd $degeneracy -co $cutoff $gfc $chaosfl -pairs $tempdir/$$.anchs.pairs > $tempdir/$$.anchtemp";
...
# FIND MAXIMAL-SCORING CONSISTENT CHAIN
    ...
    `$lagandir/anchors $tempdir/$$.anchtemp $gfc $anchparams | sort -n > $tempdir/$$.anchs.sorted`;
    if ($?) { exit(1); }
...
# READ SORTED ANCHORS TO @anchors
    ...
    open(SFILE, "$tempdir/$$.anchs.sorted");
    @anchors = <SFILE>;
    close(SFILE);
	...
	#print STDERR "\nprevanchorscore is: $prevanchorscore\n";
...
$res = `sort -nr $tempdir/$$.anchs.sorted`;
if ($?) { exit(1); }
