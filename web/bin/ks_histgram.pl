...
use Getopt::Long;
use POSIX;
use vars qw($help $ks_db $ks_type $outfile $width $height $log $pair_file $chr1 $chr2);
...
sub gen_color_list
  {
	...
	push @colors, get_color_new(val=>$val);
      }
    return \@colors;
  }
