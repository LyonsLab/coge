...
use vars qw($DEBUG $GO $conf_file $coge @gids $restricted $P $org_name $version);
...
my $new_faa = get_and_clean_sequence(genomes=>\@genomes, chr_headers=>$chr_headers);

my $stid = get_sequence_type_id_for_merged();

print STDERR "Loading genome\n";
my $new_gid = load_genome (faa=>$new_faa, genomes=>\@genomes, stid=>$stid, org_name=>$org_name, version=>$version);
...
add_annotations(new_gid=>$new_gid, genomes=>\@genomes, chr_headers=>$chr_headers);
	...
	foreach my $chr (sort keys %$ds)
	  {
	    ...
	    unless ($new_ds->{$new_chr})
	      {
		print STDERR "Chromosome $new_chr does not exist in new genome.  Please check the chromosome names if you think this is in error.  Skipping to next chromosome\n";
		next;
	      }
	    foreach my $f1 ($ds->{$chr}->features->search({chromosome=>$chr}))
	      {
		...
		if ($f2)
		  ...
		  {
		    ($f2)= replicate_feature (f1=>$f1, ds2=>$new_ds->{$new_chr}, new_chr=>$new_chr);
		  }
...
sub replicate_annotations
  {
	...
	$f2->add_to_feature_annotations({annotation=>$anno->annotation, annotation_type_id=>$anno->annotation_type_id}) if $GO;
      }
  }
...
#    $cmd .= " -genome_name '". $dsg->name."'" if $dsg->name;
    my $genome_name = join ("_".map{$_->id} sort {$a->id <=> $b->id} @$genomes);
    my $message = "Merged genome of: ";
    $message .= join (", ", map{"<a href=OrganismView.pl?gid=".$_->id." target=_new>".$_->organism->name."</a>"} sort {$a->id <=> $b->id} @$genomes);
...
sub get_and_clean_sequence
  {
	...
	unless ($header)
	  {
	    print STDERR "Warning: No header for ".$genome->id. ".  Skipping\n";
	    next;
	  }
...
sub get_sequence_type_id_for_merged
  {
    my $st = $coge->resultset('GenomicSequenceType')->find_or_create({name=>"Megered genomes"});
    return $st->id;
  }
