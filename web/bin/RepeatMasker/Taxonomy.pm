...
#
#     $Log: Taxonomy.pm,v $
#     Revision 1.56  2007/05/17 21:01:54  rhubley
...
sub predates {
  ...
  # Make sure ancestrale species are not a synonyms
  my $ancSpecies1ID = -1;
...
sub _buildFromNCBIDB {
  ...
  if ( defined $RMDatabase ) {
    ...
    #my $db =
    #    FastaDB->new( fileName => $RMDatabase, openMode => SeqDBI::ReadOnly );
    #my @descs = $db->getDescriptors();
    #undef $db;
    my $db = RepbaseEMBL->new( fileName => $RMDatabase );
    ...
    #print "  - Adding all children of the seeded list\n";
    while ( @childlist ) {

#print "    -- Number of entries in child list = " . $#childlist . "\n";
#print "    -- Number of entries in goodTaxIDs = " . scalar ( keys %goodTaxIDs ) . "\n";
      my $id = pop @childlist;
