...
sub writeEMBLFile {
  ...
  open OUT, ">$fileName"
      || die "$CLASS::writeEMBLFile() Unable to open "
...
sub _parseTagData {
  ...
  if ( $tag eq "ID" ) {
    ...
    if ( $data =~ /(\S+)\s+(\S+)\;\s+(\S+)\s*\;\s+(\S+)\s*\;\s+(\d+)\s+BP.*/ ) {
      ...
      croak "parseTagData: ID field not to EMBL" . " spec \"$data\"\n";
    }
  }
  elsif ( $tag eq "CC" ) {
    if ( $data =~ /RepeatMasker\s+Annotations/i ) {
      if ( $data =~ /Type:[ \t]*(\S+)[ \t]*[\n\r]/i ) {
        ...
        #print STDERR "Found type = $1\n";
      }
      if ( $data =~ /SubType:[ \t]*(\S+)[ \t]*[\n\r]/i ) {
        $recRef->setRMSubType( $1 );

        #print STDERR "Found subtype = $1\n";
      }
      ...
      if ( $data =~ /Species:[ \t]*([\S \t]+)[ \t]*[\n\r]/i ) {
        ...
        #print "Species String = $speciesStr - ";
        $speciesStr =~ s/\s//g;
        $speciesStr =~ s/(.*),$/$1/g;
      ...
      if ( $data =~ /SearchStages:\s*([\d\s*\,\s*]+)[\n\r]/i ) {
        ...
        #print STDERR "Found searchstages = $1\n";
      }
      if ( $data =~ /BufferStages:([^\n\r]*)[\n\r]+/i ) {
        ...
        while ( $bufData =~ s/\s*(\d+\s*(?:\[\s*\d+\s*-\s*\d+\s*\]\s*)*),?//m )
        {
          ...
          #print STDERR "Found bufferstages = $1\n";
        }
      }
      if ( $data =~ /Description:\s+(\S.*)/i ) {
        $recRef->setRMDescription( $1 );

        #print STDERR "Found description = $1\n";
      }
    }
...
/(\d+)-(\w+)(-\d+)?\s+\(Rel. (\d+(\.\d+)*), Last updated, Version (\d+)\)/
          )
      {
        ...
        croak "parseTagData: DT field not to EMBL" . " spec \"$_\"\n";
      }
    }
  }
  elsif ( $tag eq "DE" ) {
