...
our (
	...
	$URL,           $SERVER,        $USER,
	$FORM,          $coge,          $cogeweb,
	$PAGE_NAME,     $FORMATDB,      $BLAST,
	$TBLASTX,       $BLASTN,        $BLASTP,
	$LASTZ,         $LAST,          $DATADIR,
	$FASTADIR,      $BLASTDBDIR,    $DIAGSDIR,
	$MAX_PROC,      $DAG_TOOL,      $PYTHON,
	$PYTHON26,      $TANDEM_FINDER, $RUN_DAGCHAINER,
	$EVAL_ADJUST,   $FIND_NEARBY,   $DOTPLOT,
	$SVG_DOTPLOT,   $NWALIGN,       $QUOTA_ALIGN,
	$CLUSTER_UTILS, $BLAST2RAW,     $BASE_URL,
	$BLAST2BED,     $SYNTENY_SCORE, $TEMPDIR,
	$TEMPURL,       $ALGO_LOOKUP,   $GZIP,
	$GUNZIP,        %FUNCTIONS,     $JEX,
	$GENE_ORDER,    $PAGE_TITLE,    $KSCALC,
	$GEN_FASTA,     $RUN_ALIGNMENT, $RUN_COVERAGE,
	$GEVO_LINKS,    $PROCESS_DUPS,
	$SEQUENCE_SIZE_LIMIT, $SCRIPTDIR
);
...
$LASTZ =
  ...
  . " -A $MAX_PROC --path="
  . $config->{LASTZ};
$LAST = $config->{MULTI_LAST} . " -a $MAX_PROC --path=" . $config->{LAST_PATH};
...
sub generate_assembly {
	...
	# Submit workflow
	my $submission =
	  generate_pseudo_assembly( $JEX, $config, $opts{input}, $output, $flip );
	$output =~ s/$DIR/$URL/;
