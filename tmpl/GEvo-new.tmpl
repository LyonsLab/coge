...
<script src="./gobe/static/gobe-new.js" ></script>
<style type="text/css">
	#red, #green, #blue {
		float: left;
		clear: left;
...
/* so up is the 'left', down is the 'right' bar. */
    ...
    value = Math.min(value, updown == 'up' ? lefts[idx - 1] : rights[idx - 1]);
...
function update_line_width(val)
 {
   ...
   $('#svg').find('line').css('stroke-width', val);
 }
...
function new_left_slider(i, y, h) {
   var l = $('<canvas id="slider_up_' + (i + 1) + '" style="position:absolute;left:0;top:' + y + 'px;width:11px;height:' + h + 'px" width="' + (11 * window.devicePixelRatio) + 'px" height="' + (h * window.devicePixelRatio) + 'px"></canvas>');
    var ctx = l[0].getContext('2d');
    ctx.scale(window.devicePixelRatio, window.devicePixelRatio);
    ctx.beginPath();
    ctx.moveTo(10.5, 0);
    ctx.quadraticCurveTo(-6, 8, 8.5, 16);
    ctx.lineTo(8.5, h - 16);
    ctx.quadraticCurveTo(-6, h - 8, 10.5, h);
    ctx.closePath();
    ctx.fillStyle = '#ccc';
    ctx.fill();
...
function new_right_slider(i, x, y, h) {
    var r = $('<canvas id="slider_down_' + (i + 1) + '" style="position:absolute;left:' + x + 'px;top:' + y + 'px;width:11px;height:' + h + 'px" width="' + (11 * window.devicePixelRatio) + 'px" height="' + (h * window.devicePixelRatio) + 'px"></canvas>');
    var ctx = r[0].getContext('2d');
    ctx.scale(window.devicePixelRatio, window.devicePixelRatio);
    ctx.beginPath();
    ctx.moveTo(0.5, 0);
    ctx.quadraticCurveTo(16, 8, 2.5, 16);
    ctx.lineTo(2.5, h - 16);
    ctx.quadraticCurveTo(16, h - 8, 0.5, h);
    ctx.closePath();
    ctx.fillStyle = '#ccc';
    ctx.fill();
    ctx.strokeStyle = 'black';
...
function drag_left(event, ui) {
    ui.position.left = Math.max(0, ui.position.left);
    ui.position.left = Math.min(ui.helper[0].r.position().left - 11, ui.position.left);
    var id = ui.helper[0].id;
    var num = get_sequence_num(parseInt(id.substring(id.lastIndexOf('_') + 1)));
    var info = image_info[basename + '_' + num + '.png'];
    var width = info.bpmax - info.bpmin + 1;
    var ratio = width / info.img_width;
    Gobe.set_genespace('up', num, Math.round((info.xmin - ui.position.left - 0.5) * ratio), width, width * ratio);
}

function drag_right(event, ui) {
    ui.position.left = Math.max(ui.helper[0].l.position().left + 11, ui.position.left);
    ui.position.left = Math.min(1011, ui.position.left);
    var id = ui.helper[0].id;
    var num = get_sequence_num(parseInt(id.substring(id.lastIndexOf('_') + 1)));
    var info = image_info[basename + '_' + num + '.png'];
    var width = info.bpmax - info.bpmin + 1;
    var ratio = width / info.img_width;
    Gobe.set_genespace('down', num, Math.round((ui.position.left - 9.5 - info.xmax) * ratio), width, width * ratio);
}

function get_image_index(y) {
    for (var i=0; i<num_img; i++)
        if (y >= tops[i] && y <= tops[i] + image_height)
            return i;
    return -1;
}

function get_sequence_num(order) {
    for (var i=1; i<=num_img; i++)
        if ($('#display_order' + i).val() == order)
            return i;
}
...
function get_tops() {
    ...
    for (var i=1; i<=num_img; i++) {
        var img = $('#img' + i);
        tops.push(img.offset().top - img.parent().offset().top);
    }
}
...
function images_loaded() {
    ...
    for (var i=0; i<num_img; i++) {
        ...
        var img = $('#img' + (i + 1));
        var h = img.height() - 24;
        var l = new_left_slider(i, top, h).appendTo(div).draggable({axis:'x',drag:drag_left});
        var r = new_right_slider(i, img.width() + 11, top, h).appendTo(div).draggable({axis:'x',drag:drag_right});
        l[0].r = r;
        r[0].l = l;
        lefts.push(parseInt($('#drup' + (i + 1)).val()));
        rights.push(parseInt($('#drdown' + (i + 1)).val()));
    }
    $.ajax({
        dataType: 'json',
        url: 'gobe/gobe.py/' + basename + '/info/',
...
function svg_click(event) {
    var bbox = false;
    drag_started = false;
    if (!document.getElementById('bbox').hidden) {
        document.getElementById('bbox').hidden = 1;
        bbox = true;
    }
    var x = event.offsetX;
    var y = event.offsetY;
    Gobe.handle_html('');
    var img = get_image_index(y);
    if (img == -1)
        return;
    y -= tops[img];
    var url = 'gobe/gobe.py/' + basename + '/query/?y=' + y + '&img=' + (img + 1) + '&x=' + x;
    if (event.shiftKey || bbox)
        url += '&all=1';
    if (bbox)
        url += '&bbox=' + (Math.min(bbox_x1, bbox_x2) - 11) + ',' + (Math.min(bbox_y1, bbox_y2) - tops[img]) + ',' + (Math.max(bbox_x1, bbox_x2) - 11) + ',' + (Math.max(bbox_y1, bbox_y2) - tops[img]);
    $.ajax({
        dataType: 'json',
        url: url,
        success: function(data) {
            data = data.resultset;
            var wedges = $('#set_wedges').css('display') == 'none';
            var line_width = $('#line_width_val').val();
            var svg = document.getElementById('svg');
            for (var i=0; i<data.length; i++) {
                var pair = data[i];
                Gobe.handle_html('<p><a target="_blank" href="' + pair.link + '">full annotation</a></p>&#10;&#10;<p>' + pair.annotation + '</p>');
                if (!pair.has_pair)
                    continue;
                var color = '#' + pair.color.substring(2);
                var keys = [];
                for (key in pair.features)
                    keys.push(key);
                keys.sort();
                var coords1 = pair.features[keys[0]];
                var coords2 = pair.features[keys[1]];
                if (svg.connectors[coords1[4]])
                    continue;
                svg.connectors[coords1[4]] = true;
                var y1 = tops[keys[0].substring(3) - 1];
                var y2 = tops[keys[1].substring(3) - 1];
                var r1x1 = coords1[0] + 11;
                var r1x2 = coords1[2] + 11;
                var r2x1 = coords2[0] + 11;
                var r2x2 = coords2[2] + 11;
                var r1y = coords1[3] + y1;
                var r2y = coords2[1] + y2;
                var r1 = newSVG('rect', {x: r1x1, y: coords1[1] + y1, width: r1x2 - r1x1, height: coords1[3] - coords1[1], style: 'fill-opacity:0;stroke:black;stroke-width:0.5;', onclick: 'remove_connector(this);event.stopPropagation();'});
                var r2 = newSVG('rect', {x: r2x1, y: r2y, width: r2x2 - r2x1, height: coords2[3] - coords2[1], style: 'fill-opacity:0;stroke:black;stroke-width:0.5;', onclick: 'remove_connector(this);event.stopPropagation();'});
                var c;
                if (wedges)
                    c = newPolygon(r1x1 + ',' + r1y + ' ' + r2x1 + ',' + r2y + ' ' + r2x2 + ',' + r2y + ' ' + r1x2 + ',' + r1y, color);
                else
                    c = newLine((r1x1 + r1x2) / 2, r1y, (r2x1 + r2x2) / 2, r2y, color, line_width);
                r1.r2 = r2;
                r1.c = c;
                r2.r1 = r1;
                r2.c = c;
...
function calc_bbox() {
    var x1 = Math.min(bbox_x1, bbox_x2);
    x1 = Math.max(x1, 11);
    var x2 = Math.max(bbox_x1, bbox_x2);
    x2 = Math.min(x2, 1011);
    var y1 = Math.min(bbox_y1, bbox_y2);
    y1 = Math.max(y1, tops[bbox_img]);
    var y2 = Math.max(bbox_y1, bbox_y2);
    y2 = Math.min(y2, tops[bbox_img] + image_height);
    var div = document.getElementById('bbox');
    div.style.left = x1 + 'px';
    div.style.top = y1 + 'px';
    div.style.width = x2 - x1 + 'px';
    div.style.height = y2 - y1 + 'px';
}
function svg_mousedown(e) {
    bbox_img = get_image_index(e.offsetY);
    if (bbox_img != -1) {
        bbox_x1 = e.offsetX;
        bbox_y1 = e.offsetY;
        drag_started = true;
...
function svg_mousemove(e) {
    if (drag_started) {
        if (Math.abs(e.offsetX - bbox_x1) > 2) {
            drag_started = false;
            bbox_x2 = e.offsetX;
            bbox_y2 = e.offsetY;
            calc_bbox();
            document.getElementById('bbox').hidden = 0;
            return;
        }
    }
    if (!document.getElementById('bbox').hidden) {
...
function svg_mouseleave(e) {
    drag_started = false;
    document.getElementById('bbox').hidden = 1;
}

function remove_connector(e) {
    drag_started = false;
    if (!document.getElementById('bbox').hidden) {
        document.getElementById('bbox').hidden = 1;
        return;
    ...
    if (e.connector_id)
        delete $('#svg')[0].connectors[e.connector_id];
    var p = e.parentNode;
    if (e.r1)
        p.removeChild(e.r1);
    if (e.r2)
        p.removeChild(e.r2);
    if (e.c) {
        delete $('#svg')[0].connectors[e.c.connector_id];
        p.removeChild(e.c);
    }
    p.removeChild(e);
}

function set_connector(type) {
    $('#svg').find(type == 'line' ? 'polygon' : 'line').each(function(i, e) {
        var c;
        var r1x1 = parseFloat(e.r1.getAttribute('x'));
        var r1x2 = r1x1 + parseFloat(e.r1.getAttribute('width'));
        var r1y = parseFloat(e.r1.getAttribute('y')) + parseFloat(e.r1.getAttribute('height'));
        var r2x1 = parseFloat(e.r2.getAttribute('x'));
        var r2x2 = r2x1 + parseFloat(e.r2.getAttribute('width'));
        var r2y = parseFloat(e.r2.getAttribute('y'));
        if (type == 'line') {
            var line_width = $('#line_width_val').val();
            c = newLine((r1x1 + r1x2) / 2, r1y, (r2x1 + r2x2) / 2, r2y, e.style.stroke, line_width);
        } else
            c = newPolygon(r1x1 + ',' + r1y + ' ' + r2x1 + ',' + r2y + ' ' + r2x2 + ',' + r2y + ' ' + r1x2 + ',' + r1y, e.style.stroke);
        c.r1 = e.r1;
        c.r1.c = c;
        c.r2 = e.r2;
        c.r2.c = c;
        c.connector_id = e.connector_id;
        e.parentNode.replaceChild(c, e);
    });
}

function newLine(x1,y1,x2,y2,color,width) {
    return newSVG('line', {x1: x1, y1: y1, x2: x2, y2: y2, style: 'stroke:' + color + ';stroke-width:' + width + ';stroke-linecap:round;stroke-opacity:.6;', onclick: 'remove_connector(this);event.stopPropagation();'});
}

function newPolygon(points, color) {
    return newSVG('polygon', {points: points, style: 'fill:' + color + ';fill-opacity:0.3;stroke:' + color + ';stroke-width:0.5;', onclick: 'remove_connector(this);event.stopPropagation();'});
}
...
<div id="tabs" class="hidden">
	...
	<span class='ui-button ui-corner-all ui-button-icon-left coge-button-sm coge-button-left' onClick="add_seq(['num_seqs'],[process_add_seq]);  $('#new_drseq').remove;"><span class="ui-icon ui-icon-plus"></span>Add Sequence</span>

	<span class='r ui-button ui-corner-all ui-button-icon-left coge-button-sm coge-button-left' onClick="merge_previous(['previous_url','num_seqs'],[process_add_seq]); gen_hsp_colors(['args__num_seqs','num_seqs'],[add_hsp_colors]);"><span class="ui-icon ui-icon-plus"></span>Merge</span>
	<span class="r small" style="margin-right:1em;">Merge Previous GEvo Analysis (paste in URL): <input name=previous_url id=previous_url size="20" value="<TMPL_VAR NAME='PANEL_URL'>"/></span>
	<br><br>
	...
	<span class="small note">Change order by dragging boxes.</span><br><br>
	  <table>
	    ...
	    <span class="xsmall link padded" onclick="walk(-1)">Walk all sequences right</span>
	     ...
	     <div class="small">
	     <br>
	     Apply distance to all submissions: <input size=8 name="apply_all" id="apply_all" value="<TMPL_VAR NAME="APPLY_ALL">" />
	     &nbsp&nbsp&nbsp&nbsp
	     Pad CoGe Sequences with additional sequence: <input name="pad_gs" id="pad_gs" value="<TMPL_VAR NAME="PAD_GS">" size =8/>
...
<option value="linear" <TMPL_VAR NAME="FEAT_LABELS_LIN">>Draw Linear</option>
 ...
 <tr>
  <td> Labels font size:
  <td><input size =3 value="13" id="font_size" />
 <tr>
  <td>Don't show HSPs with <input size=3 id="hsp_overlap_limit" value="<TMPL_VAR NAME="HSP_OVERLAP_LIMIT">"/> overlaps
 <tr>
  <td colspan=2>Don't show HSPs that are shorter than <input size=3 id="hsp_size_limit" value="<TMPL_VAR NAME="HSP_SIZE_LIMIT">"/> nt. <span>(0 means no limit)</span>
 <Tr>
  <td>Draw:
...
<span class='ui-button ui-corner-all ui-button-icon-left' id="SaveSettings" onclick="<TMPL_VAR NAME="SAVE_SETTINGS">"><span class="ui-icon ui-icon-disk"></span>Save Settings</span>
<span class='ui-button ui-corner-all ui-button-icon-left' id="ResetSettings" onclick="reset_settings_gevo([],[]); window.location.reload()"><span class="ui-icon ui-icon-arrowrefresh-1-s"></span>Reset Settings </span>
</TMPL_IF>
</table>
...
<div class="padded">
  <span id="go" class='ui-button ui-button-go ui-corner-all go_run coge-button-lg' onClick="populate_page_obj(); set_display_order(); loading([],['results']); go_run();">Run GEvo</span>
</div>
</form>
</TMPL_IF>
...
<TMPL_IF NAME="SEQ_SELECT">
	<TMPL_LOOP NAME="SEQ_SELECT_LOOP">
		<div id="seq_set<TMPL_VAR NAME="SEQ_NUM">" class="coge-bg portlet" style='padding: 0.3em'>
			<div class="portlet-header">
				...
				Display Order <input name='display_order<TMPL_VAR NAME ="SEQ_NUM">' id='display_order<TMPL_VAR NAME ="SEQ_NUM">' class="display_order" size="2" value='<TMPL_VAR NAME="SEQ_NUM">' />
			</div>
	...
	<div id=skip_font<TMPL_VAR NAME="SEQ_NUM"> style="color: red;display:none"> Sequence Skipped</div>
	   <span id="seqopts<TMPL_VAR NAME="SEQ_NUM">" class="small ui-button ui-corner-all ui-button-toggle coge-button" onClick='$(".sequence_options<TMPL_VAR NAME="SEQ_NUM">").slideToggle();'>Sequence <TMPL_VAR NAME="SEQ_NUM"> Options</span>
