...
# ---------------------------------------------------------------------------------------------------------------------
# USAGE: python dotplot_dots.py <synmap_gcoords.ks_file> <OPTIONAL: options_name_>
#
# -- REQUIRED: synmap "*.aligncoords.gcoords.ks" output file.
# -- OPTIONAL: Option-based name suffix.  (currently not really used but for later implementation)
# ---------------------------------------------------------------------------------------------------------------------

log = {}

# Assign first CL argument to input_ksfile, raise error and exit if not specified.
try:
    input_ksfile = argv[1]
    output_dir = path.dirname(path.abspath(input_ksfile))
except IndexError:
...
try:
    option_name = argv[2]
    # Append "_" to underscore if absent.
    if option_name[-1] != "_":
