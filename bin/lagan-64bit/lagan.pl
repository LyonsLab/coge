...
if ($rundraft){
 ...
 #print STDERR "perl $lagandir/rechaos.pl $firstName $secondName $gfc $arglist > $tempdir/$$.anchs.final\n";
`perl $lagandir/rechaos.pl $firstName $secondName $gfc $arglist > $tempdir/$$.anchs.final`;
...
if ($ex_val) { exit(1); }
if ($contigflag){
    @bounds = `$lagandir/utils/getbounds $tempdir/$$.anchs.final $firstName $secondName`;
    if ($?) { exit(1); }
    chomp $bounds[0];
    print STDERR ("Aligning with bounds: $bounds[0]\n");
    print `$lagandir/order $firstName $secondName $bounds[0] $orderfl -anc $tempdir/$$.anchs.final`;
    if ($?) { exit(1); }
}
else {
    if ($dofastreject){
	...
	open(SFILE, "$tempdir/$$.anchs.final");
	@anchors = <SFILE>;
	close(SFILE);
    ...
    `$lagandir/order $firstName $secondName $format -out $tempdir/$$.align $orderfl -anc $tempdir/$$.anchs.final`;
    if ($?) { exit(1); }

    if (!$okformat) {
	if ($filename) {
	    `$lagandir/utils/bin2bl $tempdir/$$.align > $filename`;
	}
	else {
	    print `$lagandir/utils/bin2bl $tempdir/$$.align`;
	}
    }
    else {
	if ($filename) {
	    `cat $tempdir/$$.align > $filename`;
	}
	else {
	    print `cat $tempdir/$$.align`;
	}
    }
    if ($dofastreject){
	`$lagandir/utils/scorealign $filename $consrate -ibounds -cropxmfa > $tempdir/$$.temp`;
	if ($?) { exit(1); }
	`mv $tempdir/$$.temp $filename`;
    }
}
