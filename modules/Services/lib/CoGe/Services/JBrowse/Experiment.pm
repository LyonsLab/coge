...
sub features {
	...
	foreach my $exp (@experiments) { #TODO need to move this code along with replicate in bin/fastbit_query.pl into CoGe::Web sub-module
		...
		my $cmd = "/usr/local/bin/ibis -v 1 -d $storage_path -q \"select chr,start,stop,strand,value1,value2 where 0.0=0.0 and chr='$chr' and start > $start and stop < $end order by start limit 999999999\" 2>&1"; # mdb added 3/27/13 issue 61
		#print STDERR "$cmd\n";
		my @cmdOut = qx{$cmd};
		#print STDERR @cmdOut;
		my $cmdStatus = $?;
...
#	my $start = $self->query->param('start');
#	my $end = $self->query->param('end');
#	print STDERR "experiment features eid=" . ($eid ? $eid : '') . " $chr:$start:$end\n";
#	
#	my $exp_storage_path = '/storage/coge/data/experiments/0/0/0/'.$eid; #FIXME hardcoded path
#	
#	# Call FastBit to do query
#	# Issue 61: query string must contain a "." for fastbit to use consistent
#	# format (see jira thread for explanation) so "0.0=0.0" was added along with
