...
#	my $numFeatures = 0;
	foreach my $exp (@experiments) { #TODO need to move this code along with replicate in bin/fastbit_query.pl into CoGe::Web sub-module
		...
		print STDERR "matt: " . $exp->name . " " . $exp->data_type . "\n";
		...
		if (!$data_type or $data_type < 2) { #FIXME hardcoded data_type to "quant"
			...
			foreach (@cmdOut) {
				...
				if (/^\"/) { #if (/^\"$chr\"/) { # potential result line
					s/"//g;
					my @items = split(/,\s*/);
					next if (@items != $NUM_QUANT_COL);# || $items[0] !~ /^\"?$chr/); # make sure it's a row output line
					...
					my $eid = $exp->id;
					$results .= ($results ? ',' : '') . qq{{ "id": $eid, "start": $start, "end": $end, "score": $value1, "score2": $value2 }};
					#$numFeatures++;
				}
			}
		}
			...
			# Call FastBit to do query (see issue 61: query string must contain a "." for fastbit to use consistent output)
			my $cmd = "$CMDPATH -v 1 -d $storage_path -q \"select chr,start,stop,type,id,ref,alt,qual,info where 0.0=0.0 and chr='$chr' and start <= $end and stop >= $start order by start limit 999999999\" 2>&1";
			print STDERR "$cmd\n";
			my @cmdOut = qx{$cmd};
			print STDERR @cmdOut;
			my $cmdStatus = $?;
			...
			foreach (@cmdOut) {
				...
				if (/^\"/) { #if (/^\"$chr\"/) { # potential result line
					s/"//g;
					my @items = split(/,\s*/);
					next if (@items != $NUM_VCF_COL);# || $items[0] !~ /^\"?$chr/); # make sure it's a row output line
					for (my $i = 0; $i < @items; $i++) {
						$items[$i] = 1 if $items[$i] !~ /\w/; # what's this for?
					}
					#$results .= '[' . join(',', map {"\"$_\""} @items) . ']';
					
					my ($chr, $start, $end, $type, $id, $ref, $alt, $qual, $info) = @items;
					$end = $start+1 if ($end == $start); #FIXME revisit this
					my $eid = $exp->id;
					$results .= ($results ? ',' : '') . qq{{ "id": $eid, "name": "blah", "start": $start, "end": $end, "score": $qual }};
					#$numFeatures++;
				}
			}
		}
