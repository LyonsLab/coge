...
sub build {
    ...
    if ( $file_type eq 'fastq' || $file_type eq 'bam' ) {
        ...
        my $alignment_metadata;
        if ( $file_type && $file_type eq 'fastq' ) {
            # Add alignment workflow
            my $alignment = CoGe::Builder::Common::Alignment::build(
                user => $self->user,
            ...
            return if ($alignment->{error}); #TODO need to propagate this error up to client
            
            $alignment_metadata = $alignment->{metadata};
            push @tasks, @{$alignment->{tasks}};
            $bam_file = $alignment->{bam_file};
            push @done_files, @{$alignment->{done_files}};
            $result_count++;
        }
        ...
        if ($self->options->{notebook}) {
            ...
            push @additional_md, @$alignment_metadata if ($alignment_metadata);
