package CoGe::Builder::Tools::DotplotsDots;
...
sub build {
	...
	$self->workflow( $self->jex->create_workflow( name => "DotplotsDots", init => 1 ) );
	...
	$self->workflow->add_job(
		{
			cmd         => catfile($SCRIPTDIR, 'dotplots_dots.py') . ' ' . $ksfile,
			outputs     => [catfile($DIAGSDIR, $dir1, $dir2, 'log.json')],
			description => "running dotplots_dots...",
		}
	);
