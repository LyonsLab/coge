...
sub create_load_vcf_job {
    ...
    return {
        ...
        args => [
            ...
            ['-name', "'".escape($metadata->{name})." (SNPs)". "'", 0],
            ['-desc', "'".escape($desc)."'", 0],
            ['-version', "'".escape($metadata->{version})."'", 0],
            ['-restricted', "'".$metadata->{restricted}."'", 0],
            ['-exit_without_error_for_empty_input', 1, 0],
            ['-gid', $gid, 0],
            ['-wid', $wid, 0],
            ['-source_name', "'".escape($metadata->{source})."'", 0],
            ['-tags', qq{"$tags_str"}, 0],
...
sub create_load_experiment_job {
    ...
    return {
        ...
        args => [
            ...
            ['-name', "'" . escape($metadata->{name}) . "'", 0],
            ['-desc', "'" . escape($metadata->{description}) . "'", 0],
            ['-version', "'" . escape($metadata->{version}) . "'", 0],
            ['-restricted', "'" . $metadata->{restricted} . "'", 0],
            ['-source_name', "'" . escape($metadata->{source_name}) . "'", 0],
            ['-annotations', qq["$annotations_str"], 0],
            ['-tags', qq["$tags_str"], 0],
...
sub create_load_genome_job {
    ...
    return {
        ...
        args => [
            ...
            ['-wid', $wid, 0],
            ['-name', '"' . escape($metadata->{name}) . '"', 0],
            ['-desc', '"' . ($metadata->{description} ? escape($metadata->{description}) : '') . '"', 0],
            ['-link', '"' . ($metadata->{link} ? escape($metadata->{link}) : '') . '"', 0],
            ['-version', '"' . escape($metadata->{version}) . '"', 0],
...
sub create_load_annotation_job {
    ...
    return {
        ...
        args => [
            ['-user_name', $user->name, 0],
            ['-wid', $wid, 0],
            ['-name', '"' . ($metadata->{name} ? escape($metadata->{name}) : '') . '"', 0],
            ['-desc', '"' . ($metadata->{description} ? escape($metadata->{description}) : ''). '"', 0],
            ['-link', '"' . ($metadata->{link} ? escape($metadata->{link}) : '') . '"', 0],
            ['-version', '"' . escape($metadata->{version}) . '"', 0],
            ['-source_name', '"' . escape($metadata->{source}) . '"', 0],
...
sub create_load_bam_job {
    ...
    return {
        ...
        args => [
            ['-user_name', $user->name, 0],
            ['-name', "'" . escape($metadata->{name}) . " (BAM alignment)" . "'", 0],
            ['-desc', "'" . escape($metadata->{description}) . "'", 0],
            ['-version', "'" . escape($metadata->{version}) . "'", 0],
            ['-restricted', "'" . $metadata->{restricted} . "'", 0],
            ['-gid', $gid, 0],
