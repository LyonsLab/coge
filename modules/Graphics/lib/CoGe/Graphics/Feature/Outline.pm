package CoGe::Graphics::Feature::Outline;
use strict;
...
#    $gd->transparent($self->get_color($self->bgcolor));
	...
	my $y1 = $c-$bh;
	my $y2 = $c+$bh;
	$gd->rectangle($x1+6,$y1+6, $x2-6, $y2-6, $color);
	$gd->rectangle($x1+4,$y1+4, $x2-4, $y2-4, $black);
	$gd->rectangle($x1+2,$y1+2, $x2-2, $y2-2, $color);
	$gd->rectangle($x1,$y1, $x2, $y2, $black);
	$gd->rectangle($x1+1,$y1+1, $x2-1, $y2-1, $black);
