package CoGe::Graphics::Feature::Diamond;
use strict;
...
##testing out smaller diamond
    $gd->filledRectangle(1,1,$w, $h, $self->get_color([255,255,255]));

    my $poly1 = GD::Polygon->new;
    $poly1->addPt(0,$h_half);
    $poly1->addPt($w_half,0);
    $poly1->addPt($w,$h_half);
    $gd->filledPolygon($poly1, $self->get_color(@color));

    my $poly2 = GD::Polygon->new;
    $poly2->addPt(0,$h_half);
    $poly2->addPt($w_half,$h);
    $poly2->addPt($w,$h_half);
