...
while (my $line = <>) {
    ...
    my ($qname, $qchr, $qstart, $qstop, $qstrand, $sname, $schr, $sstart, $sstop, $sstrand) = split(/\t/, $line);
    $qchr =~ s/^0+//;
    $schr =~ s/^0+//;

    if ($seen{ $qchr . ", " . $qstart . "," . $qstop . "," . $schr . "," . $sstart . "," . $sstop }){ next; }
    $seen{ $qstart . "," . $qstop . "," . $sstart . "," . $sstop } = 1;
    
  	my $qfeat = $dsq->add_to_features({
				 feature_type_id => $cns_id,
				 start=>$qstart, stop=>$qstop,
				 chromosome=>$qchr, strand=>$qstrand,
    }) if $GO;
  	my $sfeat = $dss->add_to_features({
				 feature_type_id => $cns_id,
				 start=>$sstart, stop=>$sstop,
				 chromosome=>$schr, strand=>$sstrand,
    }) if $GO;
    ...
    my $qfeat_name = $qfeat->add_to_feature_names({
					     name=>"CNS $qname with $sname",
					    }) if $GO ;
    my $sfeat_name = $sfeat->add_to_feature_names({
					     name=>"CNS $sname with $qname",
					    }) if $GO ;
