...
my $NCPUS = 8;
my $OUTFILE = "report.%i.blast"; # becomes report.1.blast, report.2.blast, report.3.blast, etc.
my $BLAST_CMD = 'blastall -p blastn -i %s -d %s -e 1 -G 2 -E 5 -r1 -q -2 -K 5 -m 8 -W 7 -o ' . $OUTFILE;
...
my $filename =  join(".", (@dots[0 .. $#dots - $#dots >= 1 ? 0 : 1 ], "%i", $#dots >= 1 ? $dots[$#dots] : "" ));
$filename =~ s/\.$//;
