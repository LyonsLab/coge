...
BEGIN {
    ...
    @ISA     = qw (Exporter);
    @EXPORT = qw(
      get_tiered_path get_genome_file index_genome_file get_genome_seq 
      get_genome_path get_experiment_path get_experiment_files 
      get_experiment_data create_experiment create_genome reverse_complement 
      get_workflow_paths
    );
...
sub get_workflow_paths {
    ...
    return unless ($user_name and $workflow_id);
...
sub create_genome {
    ...
    # Create the workflow
    my $workflow = $jex->create_workflow( name => 'Create Genome' );
...
sub _create_load_experiment_job {
    ...
    return (
        ...
        args => [
            ...
            ['-restricted', ( $metadata->{name} ? 1 : 0 ), 0],
            ['-gid', $gid, 0],
