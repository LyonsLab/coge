...
my $organism = $genome->get_organism_obj->find_or_create({name=>$head_obj->organism, #this adds "Arabidopsis thaliana"
							  description=>$head_obj->organism_long, #this adds the complete kingdom to species list
							 }); 
my $data_source = $genome->get_data_source_obj->find_or_create({name=>"NCBI",
...
my $data_information = $genome->get_data_information_obj->find_or_create({name=>"$gb_file",
									  desc=>$data_information_desc,
									  link=>"ftp://ftp.ncbi.nih.gov/genomes/Arabidopsis_thaliana/NC_003074.gbk",
...
while (my $gb_feature = $gb->get_next_feature)
  {
    my $feat_type = $genome->get_feature_type_obj->find_or_create({name=>$gb_feature->type});

    my $db_feature = $genome->get_feature_obj->create({feature_type_id=>$feat_type->id,
    ...
    foreach my $loc ($gb_feature->locations)
      {
	my $location = $genome->get_location_obj->create({feature_id=>$db_feature->id,
							  start=>$loc->start,
							  stop=>$loc->stop,
							  strand=>$loc->strand, #expect this to be "1" or "-1"
							  chromosome=>$source->chromosome, #make sure that chromosomes are in real numbers (not roman numerals [IX])
							 });
      }
	...
	if ($anno->type =~ /xref/i)
	  {
	    my $anno_type_group = $genome->get_annotation_type_group_obj->find_or_create({name=>$anno->type});
		...
		while (my $sub_anno = $xref_anno->next_Annotation() )
		  {
	       ...
	       $anno->gene =~ /gene/i ||
	       $anno->type =~ /synonym/i  ##synonyms are embedded in the /note= tag!
	      ) #these are names
	  {
	    my $feat_name = $genome->get_feature_name_obj->create({
								   name=>$anno->to_string,
	...
	elsif ($anno->type=~/translation/i) #this needs to be entered into the sequence table
	  {
	    my $seq_type = $genome->get_sequence_type_obj->find_or_create({name=>"protein",
	...
	elsif ($anno->type =~ /go/i) #go will need special treatment in the parser as the GO annotation is embedded in the /note= tag.  Remove it from the /note tag and leave everything else in the /note tag.
	  #example:
	  #go_function: nucleic acid binding [goid 0003676]
	  {
	    my $anno_type_group = $genome->get_annotation_type_group_obj->find_or_create({name=>$anno->type}); #this should be "go_function"
		...
		while (my $sub_anno = $go_cat_anno->next_Annotation() )
		  {
		    $genome->get_annotation_obj->create({
							 name=>$sub_anno->to_string(), #this should be "nucleic acid binding"
							 feature_id=>$db_feature->id,
							 annotation_type_id=>$anno_type->id
							});
		  }
	...
	else ##everything else
	  {
	    ...
	    warn "Name is longer than 255 characters.  Will be truncated in the database:\n$name\n" if length $name >255;
	    $genome->get_annotation_obj->create({
						 name=>$name,
						 feature_id=>$db_feature->id(),
						 annotation_type_id=>$anno_type->id(),
